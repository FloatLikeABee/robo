<div class="flex min-h-0 flex-1 flex-col gap-4">
	<h1 class="text-2xl font-bold">Help</h1>
	<div class="max-w-2xl space-y-3 text-text-secondary">
		<p>
			<strong class="text-text-primary">Data Access</strong> lets you manage
			<strong class="text-text-primary">data tables</strong> and build
			<strong class="text-text-primary">data reports</strong> from uploaded files (CSV, TSV, JSON, Excel).
		</p>
		<p>
			Start the Data Access API if the app cannot reach the backend. Chat lives in Morph AI.
		</p>
	</div>
</div>
