...
`LoadFromEnv` reads `MORPH_AI_PROVIDER`. A known id (`xai`, `openai`, `anthropic`, `gemini`, `openrouter`, `ollama`, `dashscope`, `mistral`, `groq`, `openai-compatible`), with surrounding space and any case, selects that provider. The call then uses that provider's own key and base URL env vars. The legacy `MORPH_AI_API_KEY` / `MORPH_AI_BASE_URL` / `MORPH_AI_API_URL` / `GEMINI_*` / `TRAN_QWEN_*` snapshot is not sent. A missing provider key returns `ErrProviderNotConfigured`. An unknown id returns `ErrUnknownProvider` on the first call and does not fall back to DashScope. Empty or whitespace leaves the legacy client unchanged.
...
When `MORPH_AI_PROVIDER` is set, `MORPH_AI_MODEL` is the chat model if it is set to something other than the DashScope default. `qwen3-max` (the value shipped in `.env.example`, and the value Morph writes back when that variable is unset) is not sent to a non-DashScope provider that has its own default; the provider's first suggested model is used instead. DashScope keeps `qwen3-max`. `GEMINI_MODEL` and `TRAN_QWEN_MODEL` are not used once a provider is selected. A `Config` built in code with `Model` set, including `qwen3-max`, is still sent.
