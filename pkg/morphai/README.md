...
| `groq` | OpenAI chat completions | `GROQ_API_KEY` | `https://api.groq.com/openai/v1` |
...
`LoadFromEnv` stores the DashScope compatible-mode URL even when `MORPH_AI_BASE_URL` is unset. That default is ignored when the selected provider is not DashScope or `openai-compatible`, so it is not sent to OpenAI.
...
| Legacy vision model | `qwen3.6-plus` |
| Chat timeout | 120s |
