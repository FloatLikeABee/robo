...
`LoadFromEnv` stores the DashScope compatible-mode URL even when `MORPH_AI_BASE_URL` is unset. That default is ignored when the selected provider is not DashScope or `openai-compatible`, so it is not sent to OpenAI. The key `LoadFromEnv` copies from `MORPH_AI_API_KEY`, `GEMINI_API_KEY`, or `TRAN_QWEN_API_KEY` is the same kind of default: it is used only while `Provider` is empty. Setting `Provider` to anything else drops that copied key. The call then uses a key the caller set on `Config` or on that call, or that provider's own env var. `GEMINI_API_KEY` does not authorize explicit DashScope. `TRAN_QWEN_API_KEY` and `MORPH_AI_API_KEY` still do, because those are DashScope env vars, not the copied field.
