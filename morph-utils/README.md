...
When the deploy is live, copy the HTTPS URL Render shows. `GET /health` returns `ok`. The placeholder for story #106 is `https://<morph-utils public host>`. #106 sets that origin as `REACT_APP_MORPH_UTILS_URL` on Morph. This Blueprint section does not set `REACT_APP_MORPH_UTILS_URL`. Recording that URL and the later Morph image rebuild are under `MorphUtils stack on Render` in [`deploy/README.md`](../deploy/README.md). The placeholder for story #114 is `https://<sharpreport public host>`. This change does not set `VITE_DATAX_URL`.
