...
`PORT` is `3040`. `VITE_MORPH_API_URL` is required and is not a secret: set it to `https://<morph public host>` (no path), then restart so `/config.js` is rewritten. `VITE_USERS_PANEL_API_URL` is the optional alias, used only when the primary is unset or blank. Set the embed prompts on this service. Event Logs is the `formx` service. Content Maker is the `composerx` service. Data Access is the `sharpreport` service. The Data Access placeholder is `https://<sharpreport public host>`. Project is the `morph-engi` service. The Project placeholder is `https://<morph-engi public host>`. Each key below is `sync: false` with no value in git:
...
When the deploy is live, copy the HTTPS URL Render shows. `GET /health` returns `ok`. The placeholder is `https://<morph-utils public host>`. The `morph` service lists `REACT_APP_MORPH_UTILS_URL` as a dashboard prompt with no value. This MorphUtils section does not set that key on `morph-utils`. Recording the origin and the later Morph image rebuild are under `MorphUtils stack on Render` in [`deploy/README.md`](../deploy/README.md). Fill `VITE_DATAX_URL` on `morph-utils` with the dashboard origin. The placeholder is `https://<sharpreport public host>`.

The session cookie is `userspanel_session_token`, `SameSite=Lax`, with no `Domain`. `*.onrender.com` hosts do not share a parent cookie. The shell passes `userspanel_token` into each iframe. `bk` and invite-signup are not required. Cookie failure modes are in [`deploy/README.md`](../deploy/README.md).
