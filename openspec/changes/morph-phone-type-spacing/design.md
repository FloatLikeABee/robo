...
`MuiButton` `sizeSmall` gets `minHeight: 44` and `minWidth: 44` inside the phone query so "New" on a notes surface meets 44px without a new component. `body2` at `1rem` makes "No notes yet…" and "Nothing here yet" readable, and `overflow-wrap` on `.MuiTypography-body2` lets the sentence wrap in the list column.
