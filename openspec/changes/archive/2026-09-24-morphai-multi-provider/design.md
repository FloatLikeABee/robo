...
`LoadFromEnv` always stores the DashScope compatible-mode URL in `BaseURL`. Selecting `openai` on that struct would otherwise POST the OpenAI key to DashScope. A base URL equal to that default is treated as unset for every provider except `dashscope` and `openai-compatible`.
