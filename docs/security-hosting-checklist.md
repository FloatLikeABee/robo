...
- [ ] `MORPH_SECRETS_KEY` is 32 random bytes of standard base64 (`openssl rand -base64 32`) before any provider API key is stored. It is not derived from `JWT_SECRET`. Rotation uses `MORPH_SECRETS_KEY_PREVIOUS`, which startup loads when set. See `docs/agents/14-secrets-key.md`. After the Tran data directory exists, production (`MORPH_ENV=production` or `prod`) refuses to start when `MORPH_SECRETS_KEY` is missing or invalid. The error names the variable and does not print the value. Local mode uses or creates `morph-secrets.key` (mode `0600`) in that directory and logs a warning with the path only.
