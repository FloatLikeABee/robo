...
- [ ] `MORPH_SECRETS_KEY` is 32 random bytes of standard base64 (`openssl rand -base64 32`) before any provider API key is stored. It is not derived from `JWT_SECRET`. Rotation uses `MORPH_SECRETS_KEY_PREVIOUS`. See `docs/agents/14-secrets-key.md`. Startup does not load this key yet.
