...
- [ ] `JWT_SECRET` is a unique random string of at least 32 characters. Do not keep the development value from `.env.example`. Project reads the same variable; if it is unset, Project falls back to its own development secret.
