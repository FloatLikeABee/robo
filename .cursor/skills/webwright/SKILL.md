---
name: webwright
description: Use when the user asks to autotest a web UI, verify a changed browser flow with Playwright evidence, automate a web task (search, filter, form-fill, multi-step flow, data extraction), or wants reusable scripts plus screenshots rather than a one-shot answer. Also use for /webwright:run and /webwright:craft.
allowed-tools: Bash, Read, Write, Edit, bash, read_file, write_file, Shell, StrReplace
---

# Webwright (Cursor / Claude Code adaptation)

Installed from [microsoft/Webwright](https://github.com/microsoft/Webwright) (MIT).
**In Cursor, read [cursor-host.md](cursor-host.md) first** (Shell vs Bash, autotest workspace).
...
Playwright workspace. In Cursor, **you replace that loop directly**: use the
`Shell` tool the same way the `bash_command` field is used in
...
```bash
python3.12 -m playwright install firefox
```

No API keys needed for this skill. In this environment Playwright 1.62 + Firefox
are already installed for `python3.12`.
...
- `cursor-host.md` — Cursor tool map, `webwright-runs/` autotest workspace,
  `python3.12` Playwright.
